...
$('#plant').change(function() {
    ...
    $('#getSearchResult').click(function(){
        ...
        var monthSelct = $('#month :selected').val();
        ...
        $.ajax({
            type: "POST",
            url: API_URL + '/report/ByDate_scanTrendCustomer',
            data: JSON.stringify(dataString),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            cache: false,
    ...
    if(monthSelct != '') 
        ...
        $.ajax({
            type: "POST",
            url: API_URL + '/report/ByDate_scanTrendCustomerDownload',
            data: JSON.stringify(dataString),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            cache: false,
...
*/
   ...
   // alert(subGroupId);
    ...
    $(document).on("change","#month",function() {
        ...
        var monthDay = $(this).find(':selected').val();
        var years = $("#filterYear").find(':selected').val();

        var monthDays = getDaysInMonth(monthDay, years);
         ...
         $('.monthCount').hide();   
       // var rr =$(".addMonth").append('<div class="th"><div class="subm">Totals</div> <div class="subh"><span>Nos</span><span>Amt</span></div></div>');

     //   console.log(rr);
       var mainCatOption = '<div class="th monthCount"><div class="subm">Total</div> <div class="subh"><span>Nos</span><span>Amt</span></div></div>';
      
       // const monthData = ["1", "2", "3"];
       var j=1;
        $.each(monthData, function(i, object){
       
           var ii = String(j).padStart(2, '0')
            mainCatOption +='<div class="th monthCount"><div class="subm">'+ii+'</div> <div class="subh"><span>Nos</span><span>Amt</span></div></div>';
        j++;  
        });
        $('.addMonth').append(mainCatOption);
