...
$('#plant').change(function() {
    ...
    function initData(dataArray) {
        ...
        $.each(dataArray, function(i, object) {
            ...
            r += '<div class="td inlineText">' + object.module + '</div>';
    ...
    function getarr (startDate, endDate)  {
        const dates = []
    ...
    $('#getSearchResult').click(function(){
        ...
        $.ajax({
            type: "POST",
            url: API_URL + '/report/ByDate_scanTrendModule',
            data: JSON.stringify(dataString),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            cache: false,
            ...
            success: function(response) {
                if (response.success == 1) {
                    ...
                    const d1 = new Date(response.startdate);
      const d2 = new Date(response.enddate);
      
      console.log(getDatesInRange(d1, d2));
                 //  arrofdate =  getarr(response.startdate,response.enddate,'','');
                  // console.log(getarr(response.startdate,response.enddate));
    ...
    $('#getDownload').click(function(){
        ...
        $.ajax({
            type: "POST",
            url: API_URL + '/report/ByDate_scanTrendModuleDownload',
            data: JSON.stringify(dataString),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            cache: false,
            ...
            success: function(response) {
                if (response.success == 1) {
                    ...
                    $("body").append($a);
                    $a.attr("download","ScanTrendModuleReport.xls");
                    $a[0].click();
                    $a.remove();
                } else {
                    noResult();
