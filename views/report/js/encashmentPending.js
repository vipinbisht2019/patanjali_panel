...
$('#plant').change(function() {
    ...
    function initData(response) {
        ...
        $.each(response.head, function(i, object) {
            if(i==0){
                h+='<div class="th">'+object+'</div>';
            } else {
                h+='<div class="th"><div class="subm">'+object+'</div> <div class="subh"><span>Nos</span><span>Amt</span></div></div>';
            } 
        });
        h += '</div>';
        ...
        $.each(response.data, function(i, object) {
            ...
            r += '<div class="td inlineText">' + object.title + '</div>';

            $.each(object.meta, function(i, meta) {
                r += '<div class="td"><span>'+meta.num+'</span><span>'+meta.points+'</span></div>';
            });

            r += '</div>';
        });
        datatable.find('.thead').html(h);
        datatable.find('.tbody').html(r);
    }
    ...
    $('#getSearchResult').click(function(){
        ...
        $.ajax({
            type: "POST",
            url: API_URL + '/report/encashmentPending',
            data: JSON.stringify(dataString),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            cache: false,
    ...
    $('#downloadReport').click(function(){
        ...
        var url = API_URL + '/report/encashmentPendingDownload';

        $.ajax({
            type: "POST",
            url: API_URL + '/report/encashmentPendingDownload',
            data: JSON.stringify(dataString),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            cache: false,
